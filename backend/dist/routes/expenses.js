...
router.post("/bulk", async (req, res) => {
    try {
        ...
        const created = await db_1.default.$transaction(expenses.map((item) => db_1.default.expense.create({
            data: {
                userId: defaultUser.id,
...
router.put("/:id", async (req, res) => {
    try {
        ...
        const existing = await db_1.default.expense.findUnique({
            where: { id },
...
router.delete("/:id", async (req, res) => {
    try {
        const { id } = req.params;
        const existing = await db_1.default.expense.findUnique({ where: { id } });
        if (!existing) {
            res.status(404).json({ error: "Expense not found" });
            return;
        }
        const billSplit = await db_1.default.billSplit.findUnique({ where: { expenseId: id } });
        if (billSplit) {
            await db_1.default.billItemAssignment.deleteMany({ where: { billSplitId: billSplit.id } });
            await db_1.default.billParticipant.deleteMany({ where: { billSplitId: billSplit.id } });
            await db_1.default.billSplit.delete({ where: { id: billSplit.id } });
        }
        await db_1.default.expense.delete({ where: { id } });
